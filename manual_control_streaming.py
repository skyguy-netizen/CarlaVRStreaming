...
from display import SettingsMenu
...
def send_camera_feed():
    ...
    now = time.time()
    if now - last_sent_time < 1/90:  # 10 FPS
        return
    last_sent_time = now
...
def receive_orientation(client, server, message):
    # print(f"Received message: {message}")
    try:
        data = json.loads(message)
        # print(data)
        # msg = json.loads(data.get('message', ''))
        world.camera_manager.rotate_vr_camera(data['pitch'], data['yaw'], data['roll'])
...
class World(object):
    def __init__(self, carla_world, hud, settings_menu, args):
        ...
        # self.control = KeyboardControl(self, args.autopilot)
        global websocket_server
        websocket_server = WebsocketServer(port=8765, host="0.0.0.0")
        websocket_server.set_fn_new_client(new_client)
        websocket_server.set_fn_client_left(client_left)
        websocket_server.set_fn_message_received(receive_orientation)
    ...
    def restart(self):
        ...
        self.camera_manager.setup_vr_camera()
...
class KeyboardControl(object):
    ...
    def __init__(self, world, start_in_autopilot):
        ...
        if isinstance(world.player, carla.Vehicle):
            ...
            self._vr_control = carla.VehicleControl()
...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        # self.BLEND_WIDTH = 30  # pixels for feather blending

        # # ORB Matcher (put outside function)
        # self.orb = cv.ORB_create(300)
        ...
        if not self._parent.type_id.startswith("walker.pedestrian"):
            self.vr_camera = (carla.Transform(carla.Location(x=0.05*bound_x, y=-0.25*bound_y, z=bound_z)), Attachment.Rigid)
            print(self.vr_camera)
            self._camera_transforms = [
                (carla.Transform(carla.Location(x=0.05*bound_x, y=-0.25*bound_y, z=bound_z)), Attachment.Rigid), # Front
                # (carla.Transform(carla.Location(x=-0.5*bound_x, y=0, z=2*bound_z), carla.Rotation(yaw=-90)), Attachment.Rigid), # Left
                # (carla.Transform(carla.Location(x=-0.5*bound_x, y=0, z=2*bound_z), carla.Rotation(yaw=90)), Attachment.Rigid), # Right
            ]
                # (carla.Transform(carla.Location(x=+1.9*bound_x, y=+1.0*bound_y, z=1.2*bound_z)), Attachment.SpringArmGhost),
                # (carla.Transform(carla.Location(x=-2.8*bound_x, y=+0.0*bound_y, z=4.6*bound_z), carla.Rotation(pitch=6.0)), Attachment.SpringArmGhost),
                # (carla.Transform(carla.Location(x=-1.0, y=-1.0*bound_y, z=0.4*bound_z)), Attachment.Rigid)]
            # self._camera_transforms = [
            #     (carla.Transform(carla.Location(x=0.4, y=-0.3, z=1.25), carla.Rotation(pitch=0)), Attachment.Rigid),  # First person
            #     (carla.Transform(carla.Location(x=0.1, y=-0.2, z=1.3), carla.Rotation(yaw=-60)), Attachment.Rigid),  # Left side view
            #     (carla.Transform(carla.Location(x=0.1, y=-0.2, z=1.3), carla.Rotation(yaw=60)), Attachment.Rigid)  # Right side view
            #     # carla.Transform(carla.Location(x=-5.5, z=2.8), carla.Rotation(pitch=-15))  # Third person
            # ]

            # self.multi_camera_transforms = [
            #     (carla.Transform(carla.Location(x=-0.5 * bound_x, y=0, z=2 * bound_z)), Attachment.Rigid),  # Front
            #     (carla.Transform(carla.Location(x=-0.5 * bound_x, y=0, z=2 * bound_z), carla.Rotation(yaw=-60)), Attachment.Rigid),  # Left
            #     (carla.Transform(carla.Location(x=-0.5 * bound_x, y=0, z=2 * bound_z), carla.Rotation(yaw=60)), Attachment.Rigid),  # Right
            # ]
        ...
        for item in self.sensors:
            ...
            if item[0].startswith('sensor.camera'):
                ...
                # bp.set_attribute('vertical_fov', str(90))
            ...
            item.append(bp)
        self.index = None
        self._image_buffer = [None, None, None]
    ...
    def rotate_vr_camera(self, pitch, yaw, roll):
        if not hasattr(self, 'vr_cam_sensor') or self.vr_cam_sensor is None:
            print("VR camera sensor not spawned yet")
            return

        current_transform = self.vr_cam_sensor.get_transform()
        # print("location: ", location)
        bound_x = 0.5 + self._parent.bounding_box.extent.x
        bound_y = 0.5 + self._parent.bounding_box.extent.y
        bound_z = 0.5 + self._parent.bounding_box.extent.z
        new_transform = carla.Transform(carla.Location(x=0.05*bound_x, y=-0.25*bound_y, z=bound_z), carla.Rotation(pitch, yaw, roll))
        print(new_transform)
        # Apply to camera
        self.vr_cam_sensor.set_transform(new_transform)
        # print(f"Rotated VR camera to pitch={new_rotation.pitch:.1f}, yaw={new_rotation.yaw:.1f}, roll={new_rotation.roll:.1f}")
    ...
    def setup_vr_camera(self):
        print("Calling setup")
        world = self._parent.get_world()
        blueprint = next(item[-1] for item in self.sensors if item[0] == 'sensor.camera.rgb')
        self.vr_cam_sensor = world.spawn_actor(blueprint, self.vr_camera[0], attach_to=self._parent, attachment_type=self.vr_camera[1])
        weak_self = weakref.ref(self)
        self.vr_cam_sensor.listen(lambda image: self._vr_image_callback(weak_self, image))
        print("Setup callback")
        
        # self.multi_cameras = []
        # self.multi_images = [None, None, None]

        # for i, (transform, attach_type) in enumerate(self._camera_transforms):
        #     cam = world.spawn_actor(blueprint, transform, attach_to=self._parent, attachment_type=attach_type)

        #     weak_self = weakref.ref(self)
        #     def make_callback(index):
        #         return lambda image: CameraManager._multi_image_callback(weak_self, image, index)

        #     cam.listen(make_callback(i))
        #     self.multi_cameras.append(cam)
        # print(self.multi_cameras)
    ...
    def set_sensor(self, index, notify=True, force_respawn=False):
        ...
        if needs_respawn:
            ...
            print(self._camera_transforms[self.transform_index][1])
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            #     img = array  # Keep as NumPy array for OpenCV
            #     if self.transform_index in [0, 1, 2]:  # Store images from first three transforms
            #         self._image_buffer[self.transform_index] = img
            #     # Check if all three images are available
            #     if all(img is not None for img in self._image_buffer):
            #         # Ensure all images have the same height
            #         max_height = max(img.shape[0] for img in self._image_buffer)
            #         resized_images = [
            #             cv2.resize(img, (img.shape[1], max_height), interpolation=cv2.INTER_AREA)
            #             if img.shape[0] != max_height else img
            #             for img in self._image_buffer
            #         ]
            #         # Stitch images horizontally using OpenCV
            #         stitched_img = cv2.hconcat(resized_images)
            #         # Encode to JPEG
            #         success, buffer = cv2.imencode('.jpg', stitched_img, [int(cv2.IMWRITE_JPEG_QUALITY), 50])
            #         if success:
            #             image_queue.put(buffer.tobytes())
        if self.recording:
            image.save_to_disk('_out/%08d' % image.frame)

    
    @staticmethod
    def _vr_image_callback(weak_self, image):
        self = weak_self()
        if not self:
            return

        # print(self.vr_camera[0].rotation)
        # self.vr_camera[0].rotation.yaw += 1
    ...
    #     # array = array[:, :, ::-1]  # Convert to BGR
        ...
        # curr_time = time.time()
        image_queue.put(buffer.getvalue())
        # print("Sent")

    # @staticmethod
    # def _multi_image_callback(weak_self, image, index):
        # self = weak_self()
        # if not self:
        #     return
    ...
    #     array = np.frombuffer(image.raw_data, dtype=np.uint8).reshape((image.height, image.width, 4))[:, :, :3]
    #     # array = array[:, :, ::-1]  # Convert to BGR
    #     self.multi_images[index] = array

        

    #     # stitch
    #     if all(img is not None for img in self.multi_images):
    #         order = [1, 0, 2]
    #         ordered_images = [self.multi_images[i] for i in order]

    #         # max_height = max(img.shape[0] for img in self.multi_images)
    #         # resized_images = [
    #         #     cv2.resize(img, (img.shape[1], max_height), interpolation=cv2.INTER_AREA)
    #         #     if img.shape[0] != max_height else img
    #         #     for img in ordered_images
    #         # ]
    #         stitched_img = cv.hconcat(ordered_images)

    #         success, buffer = cv.imencode('.jpg', stitched_img, [int(cv.IMWRITE_JPEG_QUALITY), 50])
    #         # img.save(buffer, format="JPEG", quality=50, optimize = True)
            
    #         # stitcher = cv.Stitcher.create()
    #         # status, pano = stitcher.stitch(ordered_images)

    #         # if status == cv.Stitcher_OK:
    #         #     # print("Can't stitch images, error code = %d" % status)
    #         #     # sys.exit(-1)
    #         #     image_queue.put(pano.tobytes())
...
def game_loop(args):
    ...
    try:
        ...
        if joystick_count >= 1:
            ...
            steering_config = (controller.steering_mode, controller.steering_sensitivity_min, controller.steering_sensitivity_max)
            if joystick_count > 1:
        ...
        # controller = SteeringwheelController(world, args.autopilot)
...
def main():
    ...
    try:
        threading.Thread(target=camera_feed_loop, daemon=True).start()
        game_loop(args)
